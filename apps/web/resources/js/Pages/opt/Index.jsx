...
export default function Index({ opts, search: initialSearch, flash }) {
    ...
    const openEdit = (opt) => {
        clearErrors();
        setData("nama_opt", opt.nama_opt);
        setSelected(opt);
        setModalType("edit");
    };
    const openDelete = (opt) => {
        setSelected(opt);
        setModalType("delete");
    };
    const closeModal = () => {
    ...
    const handleStore = (e) => {
        e.preventDefault();
        post(route("opt.store"), { onSuccess: closeModal });
    };
    const handleUpdate = (e) => {
        e.preventDefault();
        put(route("opt.update", selected.id), { onSuccess: closeModal });
    };
    const handleDestroy = () => {
        router.delete(route("opt.destroy", selected.id), {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        Kelola daftar Organisme Pengganggu Tumbuhan (OPT) yang
