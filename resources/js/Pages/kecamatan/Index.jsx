...
import { MapPin, Plus, Pencil, Trash2, X, Check, AlertTriangle, Search, ChevronLeft, ChevronRight } from 'lucide-react';

export default function KecamatanIndex({ kecamatans, search: initialSearch, flash }) {
    ...
    const handleSearch = useCallback((value) => {
        ...
        const timer = setTimeout(() => {
            router.get(route('kecamatan.index'), { search: value }, {
                preserveState: true,
                preserveScroll: true,
                replace: true,
            });
                                ...
                                rows.map((kec, idx) => (
                                    <tr key={kec.id} className="hover:bg-slate-50 transition">
                                        ...
                                        <td className="px-5 py-3.5 text-slate-500">
                                            {kec.created_at ? new Date(kec.created_at).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}
                                        </td>
                                        <td className="px-5 py-3.5 text-right">
                                            <div className="flex justify-end gap-2">
                                                <button onClick={() => openEdit(kec)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition">
                                                    <Pencil className="w-3.5 h-3.5" />Edit
                                                </button>
                                                <button onClick={() => openDelete(kec)} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-lg transition">
                    ...
                    {meta && meta.last_page > 1 && (
                        <div className="flex items-center gap-1.5">
                            ...
                            {pages[0] > 1 && (
                                <>
                                    <button
                                        onClick={() => router.get(route('kecamatan.index'), { search: searchValue, page: 1 }, { preserveState: true })}
                            ...
                            {pages.map((page) => (
                                <button
                                    key={page}
                                    onClick={() => router.get(route('kecamatan.index'), { search: searchValue, page }, { preserveState: true })}
                            ...
                            {pages[pages.length - 1] < meta.last_page && (
                                <>
                                    ...
                                    <button
                                        onClick={() => router.get(route('kecamatan.index'), { search: searchValue, page: meta.last_page }, { preserveState: true })}
                                        className="w-8 h-8 rounded-lg bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 flex items-center justify-center"
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                        <div className="flex items-center justify-between mb-5">
                            <h2 className="text-lg font-bold text-slate-900">
                                {modalType === 'create' ? 'Tambah Kecamatan Baru' : 'Edit Kecamatan'}
                            </h2>
                        ...
                        <form onSubmit={modalType === 'create' ? handleStore : handleUpdate} className="space-y-4">
                            <div>
                                ...
                                />
                                {errors.nama_kecamatan && <p className="mt-1.5 text-xs text-rose-600 font-medium">{errors.nama_kecamatan}</p>}
            ...
            {modalType === 'delete' && selected && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
                        ...
                        <h2 className="text-lg font-bold text-slate-900 mb-1">Hapus Kecamatan?</h2>
                        <p className="text-sm text-slate-500 mb-6">
                            Data <span className="font-semibold text-slate-800">"{selected.nama_kecamatan}"</span> akan dihapus permanen.
                        </p>
